...
const Ajax_Fetch = () => {
    return (
        ...
        <pre>
            ...
            React에서는 Ajex대신 fetch API 또는 Axios와 같은 라이브러리를 사용

            여기서 QNA ?! 
            fetch API : 리엑트에 내장되어 있는 API를 가져와서 사용
                API(Application Programming Interface) - 기술 연결 주소
                어떤 기술을 주소값으로 가져와 사용하거나 데이터를 보여주겠다. 할 때 사용
                ex) 마이페이지 API가 뭡니까 ? 
                (속 뜻 : 마이페이지 주소가 뭡니까 ?)
                ex) 로그인 백엔드 API가 뭡니까 ? 
                (속 뜻 : 로그인 데이터를 가져올 수 있는 백엔드 주소가 뭡니까 ?)
                ex) 게시판에서 게시물이 안보여요. API가 잘못된것 같습니다.
        ...
        <pre>
            fetch ("API주소작성") // 자바에서 try와 같은데 불러올 주소가 적힌 try
                ...
                //만약에 데이터가 무사히 들어왔다면
                .then(데이터 => { // try 안에 적힌 if문으로 만약에 어떤 데이터를 가져오는데 성공했다면
